...
for a = 1:F_num    %运行函数 F_num 8 18-23 好像显示不了，需要具体排查，先跑1-3吧
    ...
    TOC_mean_val = mean(TOC_best_score_list);
    TOC_std_val = std(TOC_best_score_list);
    ...
    SBO_mean_val = mean(SBO_best_score_list);
    SBO_std_val = std(SBO_best_score_list);
    ...
    for i = 1:30
        [AOOBest_score,AOOBest_pos, AOO_cg_curve ] = AOO(AOO_fhd,Dim,PD_no,Max_iter,LB,UB,a); % Call SFOA
        % 保存每次循环的结果
    ...
    for i = 1:30
        [PIMOBest_score,PIMOBest_pos, PIMO_cg_curve ] = PIMO(PD_no,Max_iter,LB,UB,Dim,F_obj); % Call SFOA
        % 保存每次循环的结果
