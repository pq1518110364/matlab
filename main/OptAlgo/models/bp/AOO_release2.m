function [Best_Score, Best_X, convergence_curve] = AOO_release2(fhd, dim, pop_size, iter_max, lb, ub, F_obj, varargin)
        ...
        if rand > threshold % 探索阶段：精英引导 + 差异化 + Levy 飞行
            ...
            if rand < levy_prob
                ...
                for j = 1 : dim
                    ...
                    pho_2 = X(i,:) + a.* (pbest - Best_X) + a.* (pbest - X(i,:)) ; % eq.(18)
                    % 随机选择两种更新方式之一
                    pos_n3 = X(i,:) ;
            ...
            if rand < grad_prob
                ...
                if gather_factor > 0.7  % 分散度高：直接更新强聚集
                    X(i, :) = Best_X - grad_learning_rate * grad_norm;
                else  % 分散度低：平滑过渡微调整
                    elite_updated = Best_X - grad_learning_rate * grad_norm;
