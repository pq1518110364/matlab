...
function [Best_Score,Best_X,convergence_curve]= AOOv3(fhd,dim,pop_size,iter_max,lb,ub,F_obj,varargin)
%% initialize
Best_Score=inf;
...
% 假设你已经定义了
p_best_rate = 0.1;
% 并且在循环前对Pop_Fit进行了排序，得到了sorted_index
...
for t = 1 :1:iter_max
    ...
    for i = 1 :pop_size
        ...
        if rand > threshold
            ...
            r1 = randi(pop_size);
            r2 = randi(pop_size);
            while r1 == i || r1 == r2, r1 = randi(pop_size); end
            while r2 == i || r2 == r1, r2 = randi(pop_size); end
        
            % 2. 产生一个精英个体pbest（保留了你“围绕最优”的思想）
            pNP = max(round(p_best_rate * pop_size), 2);
            pbest_index = sorted_index(randi(pNP));
            pbest = X(pbest_index, :);
        
            % 3. 增强AOO原始的W扰动，使其更具方向性
            W = c/pi *(2 * rand(1,dim) - 1) .* ub; % AOO原始扰动
            
            % 4. 融合三种信息进行更新
            % 这将取代你原始的mod策略。
            % 这里的公式可以有多种变体，例如：
            W_new = alpha * (pbest - X(i,:)) + (1-alpha)*(X(r1,:)-X(r2,:)) + W;
            X(i,:) = X(i,:) + W_new;

            % 我推荐一个更简洁的方案，直接将W方向化：
            % 50%的概率：向精英解方向移动，同时有随机扰动W
            % if rand > 0.5
            %     X(i,:) = X(i,:) + (pbest - X(i,:)) * rand + W;
            % % 50%的概率：使用两个随机个体的差异进行探索，同时有随机扰动W
            % else
            %     X(i,:) = X(i,:) + (X(r1,:) - X(r2,:)) * rand + W;
            % end
        else
            % 自适应权重，平衡梯度和随机摆动
